...
def json_send_carbon_emission(id_counter, carbon_emission, carbon_emission_total):
    ...
    json_string = r'{\"Carbon_emission\":%s,\"Carbon_emission_total\":%s,\"Photovoltaic_emission_reduction\":%s}' % (
    carbon_emission, carbon_emission_total, 0)
...
def json_send_power(id_counter, power):
    ...
    # 用generate_json_string生成的字符串替换原始字符串中的占位符
    json_string = r'{\"PowerA\":%s,\"PowerC\":%s}' % (power[0], power[1])
...
def json_send_event(id_counter, state, event_number, G=0, all_carbon_emissions=0):
    ...
    if event_number == 2 and state == 0:
        # 用generate_json_string生成的字符串替换原始字符串中的占位符
        json_string = r'{\"Electric_Kettle\":%s,\"Event_carbon_emission\":%s,\"Cumulative_carbon_emission\":%s}' % (
        state, G, all_carbon_emissions)
            ...
            with count_lock:
                if count_for_minute >= 15:
                    ...
                    if len(dianneng) >= 2:
                        ...
                        # 向TTL发送碳排数据
                        carbon_emission_string = json_send_carbon_emission(id_counter_carbon, carbon_emission,
...
def Put2base(q_col2base, q_flag, e_base, q_res2base, q_res2base_2, q_col2base_carbon_emission):
    ...
    while 1:  # 无限循环，直到程序终止
        ...
        if not q_col2base.empty():  # 检查功率数据队列是否为空
            ...
            sql = "INSERT INTO PowerData_bangongshi_0905 (DataID, PowerA, PowerB, PowerC, PowerAll, DianNeng, CarbonEmission, TimeStamp, Sourse_hex) VALUES('%d', '%.2f', '%.2f', '%.2f', '%.2f', '%.2f', '%.2f', '%s', '%s')" % (
            num_power, float(data[0]), float(data[1]), float(data[2]), float(data[3]), float(data[4]),
            float(data_carbon_emission), timestamp, data[5])
...
def Detection(q_col2de, e_det, q_res2base, minimum_power):
    ...
    # templist = pd.read_csv('DTW_Light.csv', index_col=0).values.tolist()
...
def Detection2(q_col2de_2, e_det_2, q_res2base_2, minimum_power_2):
    ...
    # templist = pd.read_csv('DTW_Lidianchi.csv', index_col=0).values.tolist()
                        ...
                        if equip != -1:
                            ...
                            if equip == 2:
                                ...
                                if state == 0 and on_count == 1:
                                    ...
                                    E_ture = E2-E1
                                    # print("电能差值：", E_ture)
                                    G_ture = E_ture*0.5422
                                    print("当前热水壶运行的实际碳排放量:", G_ture)
                                    print("当前热水壶运行算法计算的碳排放量:", G)
                                    print("碳排放量计算准确率：{:.2f}%".format((1-abs(1-G/G_ture))*100))
